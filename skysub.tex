...
\title{ANDROIDS I. Wide-Field Near-Infrared Imaging of the Andromeda Galaxy (M31)}
...
Our method differs from the original I'iwi CFHT pipeline with independent photometric, flat fielding and sky subtraction computations.
...
We find that planar sky offsets are not acceptable for subtracting these residual background across WIRCam fields that are much smaller than the mosaic area.
...
A wealth of datasets exist for M31, but none provide simultaneous observations of M31's resolved and integrated NIR light.
...
PHAT provides wide-field coverage from M31's centre to the $10$~kpc star forming ring, a panchromatic view of stellar populations in Near-UV, visible and NIR wavelengths (equivalent to a Johnson $H$), and resolution of stars in very crowded environments such as the bulge.
...
This includes: finding the optimal ST nodding cadence, defining the appropriate data reduction procedures for a WIRCam surface brightness procedure, and finally presenting an analysis of the surface brightness accuracy in wide-field WIRCam mosaics.
...
Since the sky itself is \todo{$\sim10^3\times$ brighter} than the outer disk of M31 in the NIR, a 5\% uncertainty in the background would be fatal to our objective of recovering M31's extended NIR surface brightness.
...
Additionally, we select stars with $J-K_s < 0.8$ (typical of foreground Milky Way stars) as we observe larger zeropoint residuals in redder stars.
...
Note that throughout the bright inner disk of M31, block-to-block residuals are negligible compared to the disk signal, but at the mosaic periphery ($R\sim 20$~kpc), field-to-field residuals become comparable to, or greater than, the disk surface brightness.
...
Besides the simplex method developed in \Sec{msrnm_algo}--\Sec{hierarchical_algo} and analyzed in detail in Section~\ref{fig:scalaranalysis}, we also tested the \sw{Montage} code that uses an iterative algorithm to solve either scalar or planar sky offsets (see \Sec{montage_algo}).
...
We see that planar sky offsets, in this case, do little to improve the mosaics, but have a dramatic effect on the systematic surface brightness of the mosaic (by more than 1~mag~arcsec$^{-2}$ in the $K_s$ band).
Planar sky offsets may be amenable for observations acquired in long strips (such as 2MASS or SDSS), but the small and square WIRCam fields do not provide the necessary leverage to prevent systematic error propagation from planar offsets.
...
In 2007B, all sky frames were sampled from the same coordinate on the sky, but span a 12~minute window covering sky integrations taken before and after a disk image (for both $J$ and $K_s$ sky-target nods).
...
This zeropoint can ultimately be established using resolved stellar populations, the subject of a forth coming \androids\ study.
...
Given that $\sigma_B$ cannot be reduced, increasing the number of \emph{independent} blocks (observed hours, even a night, apart to decouple sky and instrumental biases) is the most reliable way to establish the absolute surface brightness accuracy of the mosaic.
...
By doubling the number of blocks, but reducing their total integration time by half, the mosaic could be reproduced with an equivalent net integration time.
...
\bigskip We wish to thank Loic Albert and Karun Thanjavur, previously from CFHT, for their help with WIRCam I'iwi data products and procedures.
