...
\title{Andromeda (M31) Optical and Infrared Disk Survey \sc{i}.\\ Insights in Wide-Field Near-IR Surface Photometry}
...
\affil{Leibniz-Institut f\"{u}r Astrophysik Potsdam (AIP), An der Sternwarte 16, 14482 Potsdam, Germany}
\and
...
Two sky-target nodding strategies are tested, and we find that strictly minimizing sky sampling latency cannot improve background subtraction accuracy to better than 2\% of the background level due to spatio-temporal variations in the NIR skyglow.
We fully describe our WIRCam reduction pipeline and advocate using flats built from night sky images over a single night, rather than dome flats that do not capture the WIRCam illumination field.
Contamination from scattered light and thermal background in sky flats has a negligible effect on the surface brightness shape compared to the stochastic differences in background shape between sky and galaxy disk fields, which are $\sim 0.3$\% of the background level.
...
We present our WIRCam reduction pipeline and performance analysis to give specific recommendations for the improvement of NIR wide-field imaging methods.
\end{abstract}
...
The appropriate form of SFH models cannot be constrained from the integrated light of galaxies alone (as is typically attempted); resolved color-magnitude diagrams (CMDs) are both more effective and, in fact, essential at deriving non-parametric stellar population histories. 
...
While Spitzer reduces uncertainties in background estimation, the pixel scale of 0\farcs86 still prevents point source measurements of individual stars in the M31 disk.
...
Central blue tiles are the 27 disk fields observed in 2007B, surrounded by 4 sky fields (blue with solid black outlines).
Red tiles at center are the 12 disk fields observed in 2009B.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/flat_ratio_55136_Ks}
...
Note that the \texttt{NIGHT} sky flats have been renormalized to remove the chip-to-chip zeropoint correction described in \Sec{skyflatconstruction}.}
\label{fig:flatratio}
\end{figure}
...
To proceed, we must accept that our sky flats are not purely proportional calibrations, possibly up to the limit of few percent, and instead are a first step in our combined flux calibration and background subtraction pipeline.
In \Sec{flatanalysis} we analyze the combined performance of our night sky flat fielding and median background subtraction procedure to find that frame-by-frame surface brightness shape errors are dominated by rapid variations in the background itself.
...
Using a wide window, such as the full length of a queue observing run, ensures that the intrinsic WIRCam flat field function is stable over several days, while producing the statistically flattest residual sky illumination pattern.
...
We estimate the zero-points of M31's images from a sliding window average of zeropoints from adjacent sky images (analogous to the median background subtraction procedure, described in \Sec{mediansky}).
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/fw100k_betadiff}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/fw100k_chip_to_chip}
...
To analyze the shapes of these difference images we marginalize the difference images along rows (left side of \Fig{frame_residuals}), and columns (right side of \Fig{frame_residuals}).
...
Comparing panels in \Fig{frame_residuals}, we see that both \texttt{FW100K} and \texttt{NIGHT} sky flats have similar performance, where frames vary in surface brightness $\pm 0.5\%$ at the core of M31.
...
\begin{figure*}[p]
\centering
\includegraphics[width=\textwidth]{figs/frame_residuals}
\caption{Comparison of frame shape residual variations for images processed with (a) \texttt{FW100K}, (b) \texttt{NIGHT} and (c) \texttt{QRUN} sky flats. Residuals are the difference of an individual frame to the median (mosaic) shape.
Images are from the M31-37 field, $K_s$-band, observed in the 2009B semester.
...
Individual integrations are colored by their time after the first disk integration.
...
\texttt{NIGHT} sky flats reliably capture the disk shape in the signal-dominated detector \#2 as well, if not better, than \texttt{FW100K} flats.
Hence the WIRCam flat field function is stable over a night.
\texttt{QRUN} sky flats introduce large biases in the bulge-dominated surface brightness in detector \#2 (lower-right). In the more sky-dominated regions of the image (detector \#4), \texttt{QRUN} sky flats produce images with similar stability to \texttt{FW100K} and \texttt{NIGHT} sky flats, indicating that the limit of additive uncertainties associated with sky or instrumental background variations is reached here.
}
\label{fig:frame_residuals}
...
\Fig{frame_diffs_skyfrac} shows the distribution of frame-block residual shape amplitudes, measured at the 95\% difference interval to measure the reliability of measuring the surface brightness shape in individual WIRCam frames.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/framediffs_skyfrac_cdf}
...
We interpret this as \texttt{QRUN} flats introducing a bias in background shapes, while \texttt{FW100K} flats create unrealistically flat backgrounds by dividing sky glow structure that should be left for background subtraction.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/mosaics}
...
Although these image preparations can constrain the surface brightness \emph{shape} of a WIRCam frame by $\lesssim 0.3\%$ of the sky level, \Fig{mosaics}a demonstrates that the true level of the background on M31's disk is lost by the temporal and spatial variation of sky glow between disk and sky field observations.
...
% 	\caption{Scalar-sky fitted WIRCam $J$ (left) and $K_s$ (right) mosaics of M31. Note the significant qualitative improvement compared to the original, median background-subtracted, images in \Fig{mosaics}a.}
...
$\langle I_\mathrm{sky}\rangle$ is taken as the instantaneous background level for the images being sampled.}
\label{tab:offset_hierarchy}
% made with $andpipe/wircam/calibset/mosaic/scalar_offset_hierarchy.py
\input{tables/nightset_medsky_offset_hierarchy.tex}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/offsetratiomap}
...
% made with andpipe/wircam/calibset/mosaic/offsetratiomap.py
\end{figure}
...
We quantify the limited effectiveness of sky offset fitting as as the residual image level differences between coupled blocks $i$ and $j$, $(\vect{I}_i - \Delta_{B,i}) - (\vect{I}_j - \Delta_{B,j})$, after the sky offsets $\Delta_{B,i}$ have been optimally fitted to each block. 
...
\input{tables/nightset_medsky_scalar_resid_diffs.tex}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/scalar_resid}
...
Previously we have established the usefulness of sky offsets, here we address the value of 2007B and 2009B sky-target nodding observing strategies (introduced in \Sec{Observations}) when confronted with the realized sky offset distributions.
A key question is determining if the 2009B strategy to minimize background level uncertainty by minimizing the latency between sky and target samples to $\sim 1.2$~minutes is justified.
...
Here we show that the background certainty of any sky-target nodding campaign is limited both by this minimum sky sampling latency, and also be the spatial structure of the sky background.
...
This also points out why the 2009B program could only reduce the distribution of sky offsets by 30\%.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/sbdiffs}
...
%         \caption{Surface brightness difference maps between our simplex scalar-fit mosaics (\Fig{mosaics}b) and \sw{Montage} scalar-fit mosaics.}
%     \label{fig:simplex_scalarmontage_comp}
%     % make with andpipe/skysubpub/diffmaps.py
% \end{figure}
...
Despite an identical dataset, the two methods yield systematic differences of up to $\sim 0.5$~mag~arcsec$^{-2}$ at 20~kpc, though the solutions are consistent in their treatment of the inner disk.
...
% \begin{figure}[t]
% \centering
% \includegraphics[width=\columnwidth]{figs/scalar_irac36_sbdiff}
...
Strictly minimizing latency between sky and disk integrations (as in the 2009B program) provides a 30\% reduction in sky offsets, but is ultimately limited by overheads in nodding the telescope, and spatial structure in the NIR sky glow itself.
