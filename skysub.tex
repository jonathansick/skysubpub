...
This scaling obeys three requirements: 1) each image frame in the median stack is at the same level, 2) each WIRCam detector has a unified zeropoint, and 3) the sky flat across the whole array is flux normalized.\footnote{Note that it is also acceptable to establish independent zeropoints for each chip using 2MASS photometry, rather than using sky surface brightness. We verify the compatibility of the two schemes in \Sec{detector_zp}.}
