...
Monte Carlo simulations show that the absolute surface brightness level of the disk at $R=15$~kpc is uncertain by 0.1--0.2~mag~arcsec$^{-2}$; further improvements independent surface brightness calibration based on resolved stellar populations.
...
This scaling obeys three requirements: 1) each image frame in the median stack is at the same level, 2) each WIRCam detector has a unified zeropoint, and 3) the sky flat across the whole array is flux normalized.
...
Both of these elements prevent the true level of the sky on M31's disk, in any single frame, from being known to an accuracy greather than 2\%.
...
Comparing the simplex-optimized color map (\Fig{altplanar_irac36_sbdiff}) against the \sw{Montage} realization, the two algorithms find different sets of globally optimum sky offsets.
...
[something like that...]
J.S. and S.C. acknowledge support through respective Graduate Scholarship and Discovery grants from the Natural Sciences and
