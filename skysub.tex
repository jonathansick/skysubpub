...
Since the \androids\ WIRCam dataset uses revised flat fields that differ by 2\% from the \iiwione\ pipeline, new photometric zeropoints must be established. The Two Micron All Sky Survey (2MASS) Point Source Catalog \citep{Skrutskie:2006} provides a convenient photometric system to bootstrap against. Any WIRCam pointing in this survey contains $\sim 10^2$ stars measured by 2MASS. Although these are not standards, the ensemble fo 2MASS stars may be treated as such. Given the ensemble difference between instrumental and 2MASS catalog magnitudes of stars, a zeropoint is estimated. Since the disk of M31 is crowded, and 2MASS has poor resolution (seeing $> 1\arcsec$), 2MASS point source measurements are considered unreliable on the disk. Instead, all zeropoint measurements are made using sky images, and those calibrations are applied to paired disk images (analogous to the median sky subtraction procedure).

Stars are photometered with PSF-fitting photometry using \sw{DAOPHOT} \citep{Stetson:1987} and \sw{ALLSTAR} \citep{Stetson:1994}. A benefit of the PSF-fitting process is that galaxies to be culled from the 2MASS point source sample.

\sw{DAOGROW} \citep{Stetson:1990} is used to build growth curves that extrapolate the light of a star to a \emph{total} instrumental magnitude. In this survey, the median aperture correction is $\sim-0.06$ mag, so that $\sim5\%$ of the light of a star is in extended wings not captured by the PSF model.

For a typical frame, the dispersion of zeropoint estimates about the median is 0.10--0.15 magnitudes. With such an uncertainty, image-to-image variations in zeropoint can be confused with variations in sky level. But under the photometric conditions that these data were recorded, strong zeropoint fluctuations are not expected over short periods. Thus zeropoints are smoothed by averaging the zeropoints of frames across a Gaussian-weighted sliding window of width $\sigma=15$ minutes. Such time-averaged zeropoints have uncertainties of $\sim 0.05$ magnitudes.
...
\begin{figure}[t]
    ...
    \caption{Maps of $J-[3.6]$ and $K_s-[3.6]$ surface colour inferred from the scalar-sky fitted WIRCam mosaics (see \S\ref{sec:scalar}--\ref{sec:scalaranalysis}) and Spitzer/IRAC 3.6~$\mu$m image \citep{Barmby:2006}.}
...
Sky offsets produce a mosaic that is rigorously optimal only in the sense of field-to-field surface brightness continuity. The technique does not gaurantee that surface brightness is calibrated in an absolute sense. In this section, we attempt to gauge the systematic surface brightness error inherent in this technique.

\subsection{Comparison to Spitzer/IRAC Images}

A template for the old stellar disk, is the 3.6~$\mu$m Spitzer/IRAC map, presented in \cite{Barmby:2006}.
% Aside from hot dust features in the 10 kpc star forming ring, and radial color gradients, a comparison of the 3.6 \mu m surface brightness against the reconstructed $J$ and $K_s$ mosaics
In Fig. \ref{fig:scalar_irac36_sbdiff} we compare the scalar-fitted mosaics against the 3.6~$\mu$m image. Generally, $J-[3.6]$ and $K_s-[3.6]$ decreases with disk radius, but increases in the star forming regions because of hot dust emission. Both the scalar-fitted $J$ and $K_s$ mosaics are artificially reddened in the south-eastern corner, which we interpret as a systematic \emph{over-subtraction} of the WIRCam NIR sky. In $J-[3.6]$ and $K_s-[3.6]$ maps realized with planar-fitted sky, Fig. \ref{fig:altplanar_irac36_sbdiff}, a similar, though stronger, systematic sky error is seen. This result qualitatively illustrates the greater potential for sysematic sky errors when planar sky corrections are permitted to the 20 arcmin$^2$ WIRCam blocks.
...
\begin{figure}[t]
    ...
    \caption{Montage-generated $J$ and $K_s$ maps. Compare to the equivalent simplex maps, Figs. \ref{fig:scalar_mosaics} and \ref{fig:planar_mosaics}.}
...
Although this paper advocates a sky offset optimization based on the Nelder Mead Downhill Simplex algorithm, other tools exist for solving sky background corrections. A popular package is \sw{Montage} \citep{Berriman:2008}. Like the algorithms presented in this paper, \sw{Montage} uses the network of image-to-image differences to infer sky offsets. Rather than simultaneosly solve for the offsets, \sw{Montage} uses an iterative algorithm: offsets are proposed that arithmetically solves one pair of images, and that change is propagated into the adjacent pair, which are again arithmetically solved, and so forth.

Figure \ref{fig:montage_planar_mosaics} shows mosaics generated by Montage, allowing for planar sky offsets. These surface brightness realizations lead to $J-[3.6]$ and $K_s-[3.6]$ maps in Fig. \ref{fig:montage_irac36_sbdiff}. Comparing the simplex-optimized color map (Fig \ref{fig:altplanar_irac36_sbdiff}) against the \sw{Montage} realization, it is plain that the two algorithms find different sets of globally optimum sky offsets. And neither of the solutions is optimizal in the sense of absolute calibration.

Another useful comparison is Figure \ref{fig:altplanar_jk_sbdiff}, where $J-K_s$ color maps are realized by both the simplex planar-fiting and \sw{Montage} algorithms. Unlike the comparisons against $[3.6]$, these color maps test the entire \androids\ footprint. Absolute calibration of the NIR surface brightness is a clear requisite before these images are scientifically useful.
...
\begin{figure}[t]
    ...
    \caption{Maps of bootstrap RMS surface brightness in $J$ (left) and $K_s$ (right) mosaics. White contours demark RMS levels of 0.05 (solid), 0.1 (dashed) and 0.2 (dash-dot) mag arcsec$^{-2}$.}
...
\begin{figure}[t]
    ...
    \caption{Maps of dispersions in the difference fo expected and observed sky offsets in bootstrap realizations of the $J$ (left) and $K_s$ (right) mosaics. The 2009B fields are plotted over top of the 2007B fields (see Fig. \ref{fig:fieldmap}).}
    \label{fig:bootstrap_recovery_map}
    % made with skyoffset/bootstrap.py OffsetRecoveryFieldMap
\end{figure}


The difference images presented in the previous section, and particularly Fig \ref{fig:altplanar_jk_sbdiff}, illustrate how the surface brightness reconstructions of identical data can vary depending on the optimization algorithm. Here we pose a slightly different question: how are reconstructions affected by the initial conditions of sky errors? We can answer with a boostrap analysis.

A bootstrap realization is generated by perturbing the surface brightness of the corrected blocks with a sky error, drawn (with replacement) from the ensemble of block sky offsets observed in the original mosaic (Fig. \ref{fig:scalar_mosaics}). Using the scalar-sky fitting procedure, sky offsets  are optimized against the known sky perturbations. \todo{Several hundred} such realizations are made to compile an ensemble of mosaics.
