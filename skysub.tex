...
Second, we also increased the number of repetitions on each field, so that each field is observed 40 times in each band in a [ST$^2$S]$^{20}$S pattern. This repetition enables averaging over spatial sky background structures on the scale of WIRCam fields.
...
By processing all 4286 frames in the \androids /WIRCam survey simultaneously, SCAMP allows us to produce an accurate and internally consistent coordinate frame for our mosaic.
...
Wide-field sky-target nodding programs must prioritize frequent sampling of sky to track flat field evolution in WIRCam (not simply to track sky level evolution), and must have strong couplings between many blocks to enhance the effectiveness of sky offset optimization codes.
...
% First, we find little benefit in adopting an extremely rapid $STTS$ nodding cadence (our 2009B approach).
% By tracking the total offset applied to images, we see that individual 2007B images have sky uncertainties of 2.7\% in $J$ (2.4\% in $K_s$) compared to 2.0\% in 2009B.
% Simply nodding the telescope across 1\arcdeg--2\arcdeg increases sky uncertainty by $\sim 1.5$ times (see \Fig{nodding_stationary_skyvar_skyfrac}), setting the lower limit accuracy on any sky-target nodding approach.
% Spending more time on disk (up to eight minutes, as in 2007B) is valid to conserve nodding overheads.
% Note that with fewer visits to sky, additional sky images must be made in each visit so that real-time sky flats of $\sim10^5$~ADU can be built in windows of 30~minutes or less.

% We do recommend employing the randomized sky field selection used in 2009B.
% This nodding pattern was originally intended to marginalize over various possible sky shapes, theoretically producing less biased (flatter) median sky images.
% Instead we find that sky backgrounds, on the scales of WIRCam fields, are dominated by high frequency shape variations on the order of 0.2\% of the sky brightness that are caused either by sky glow or instrumental backgrounds.
% This pattern is nonetheless useful for producing sky flats with as little correlated sky and astronomical light as possible.

% Absolutely essential is the adoption of \emph{real-time} sky flats.
% Dome and twilight flats, as adopted in the CFHT/I'iwi pipeline do not recover large scale illumination, nor small scale detector structure (see \Fig{domeflatratio}).
% We also see that sky flats built from abundant data over a single queue run fail to capture the variability of the WIRCam flat field function, as evidenced by evolution in the shapes of frames TODO.
