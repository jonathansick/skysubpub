...
In particular, we are interested in the consequences of flat field design choices: how broad should the window of sky images be to reliably capture both pixel-to-pixel sensitivity, and the large scale illumination of the WIRCam detector.
...
% subsection Evolution of Real Time sky flats (end)
...
As expected by \Fig{qrun_median_sky}, \texttt{QRUN} median sky frames have typical biases of 1\% of the sky level, while sky frames from real-time \texttt{FW100K} sky flats are much flatter, $\lesssim 0.3\%$ of the sky level, and more consistently so.

Although \texttt{FW100k} sky flats are better than \texttt{QRUN} flats, we can still ask how precise they are.
...
Thus we expect the instantaneous shape of the sky (in the sky field) to be captured in the 2007B median sky frames, while the 2009B sky frames should marginalize over five random sky shapes and be flatter up to a factor of $1/\sqrt{5}$.
That this \emph{does not} occur indicates that sky shapes are correlated over tens of minutes and 3\arcdeg\ of sky, or that the flatness of median sky images is limited at the 0.3\% level by flat field variations in WIRCam (\eg \Fig{fw100k_movie}).
Indeed, because 2009B median sky images are built over wider time windows than 2007B median sky images (that are built in rapid succession, without target nod interspersed), the intrinsic flat field function of WIRCam can evolve further for 2009B median sky images, offsetting any gains from sampling uncorrelated sky shapes. \todo{Get number for time windows of 2007B and 2009B median sky windows; put in \Sec{mediansky}}.
...
But neglecting this long tail, \Fig{mediansky_amplitude} demonstrates that the combined effects of real-time \texttt{FW100K} sky flat fidelity and the intrinsic variations of the NIR sky across a $10\arcmin \times 10\arcmin$.
...
In previous sections, we have shown that real-time \texttt{FW100K} sky flats are preferable over broader-baseline queue run sky flats since they produce systematically flatter median sky images, and do seem to track real evolution in the WIRCam flat field function on scales of 30 minutes.
...
Part of this difficulty is that in observing exclusively sky fields, the amplitudes signals due to flat fielding biases and due to sky shape stochasticity will be similar.
But if we examine signal- (not sky-) dominated fields, flat fielding biases should grow into sky shape biases.
...
\caption{\todo{UPDATE THIS PLOT}. Residual shapes of individual WIRCam integrations of the M31 disk to the median (mosaic) shape for the field M31-37, $K_s$-band, 2009B semester calibrated with \texttt{FW100K} sky flats.
...
\caption{\todo{UPDATE THIS PLOT}. Same as \Fig{frame_residuals_M31-37_Ks_fw100k_medsky}, but calibrated with \texttt{QRUN} sky flats.}
...
We also extend this analysis across the entire dataset.
In \Fig{frame_residuals_M31-37_Ks_QRUN} we plot distribution of frame-block residual shape amplitudes, measured at the 95\% difference interval.
...
% Although median sky subtraction is effective at flattening the sky background shape induced by biased flat fields, we can nonetheless detect how incorrect flat fielding changes disk surface brightness.
