...
% ============================================================================
\begin{abstract}
We present wide-field near-infrared $J$ and $K_s$ images of the Andromeda Galaxy made with WIRCam on the Canada-France-Hawaii Telescope.
These images provide the highest resolution, homogeneous, dataset covering M31's entire bulge and disk (within $R=22$~kpc) to date, while simultaneously resolving stars and recovering M31's near-infrared surface brightness.
This work extensively develops and tests our WIRCam surface brightness calibrations that are complicated by the brightness of near-infrared sky glow (which is 3~dex brighter than M31's disk at $R=20$~kpc), spatiotemporal variations in the sky glow and instrumental calibrations, and by the exceptionally large area we survey.
...
Due either to atmospheric or instrumental variations, the individual WIRCam frames have typical residual shapes with amplitudes of 0.3\% of the sky after real-time flat fielding and median sky subtraction.
Two sky-target nodding strategies were tested, and we find that strictly minimizing sky sampling latency does not maximize sky subtraction accuracy, which is at best 2\% of certainty.
...
Sky offset optimizations reduce image-to-image residuals  to 0.1\% of the sky brightness.
Monte Carlo simulations show that the absolute surface brightness level of the disk at $R=15$~kpc is uncertain by 0.1--0.2~mag~arcsec$^{-2}$, necessitating independent surface brightness calibration based on resolved stellar populations.
\end{abstract}
...
Section~\ref{sec:reduction} describes the image reduction pipeline; a particularly important aspect of this is night sky flat fielding (\Sec{flats}).
In \Sec{scalar} we present our method for recovering the galaxy surface brightness by minimizing the image-to-image differences across the mosaic.
...
In \Sec{skyflatstability} we consider the accuracy of the surface brightness \emph{shapes} recovered by our pipeline; in \Sec{skybehaviour} we use our dataset to explain the nature of the NIR sky level and temporal variability.
...
Since M31, with a $190\arcmin \times 60\arcmin$ optical disk, is much larger than the WIRCam fields of view, monitoring of the sky zeropoint is only possible by periodically pointing the telescope away from M31, towards blank sky---\emph{sky-target} (ST) nodding. 

Observations were taken over two semesters, 2007B and 2009B.
The two campaigns employed different observing schemes, particularly sky-target nodding strategies.
The reader is encouraged to regard these observational designs as \emph{hypotheses} for how to best conduct a wide-field surface brightness survey in the near-infrared from the ground.
An objective of this paper is to discriminate between the virtues of the 2007B and 2009B programmes, and determine how observational design can improve the construction of a wide-field NIR mosaic.
...
The maximum nodding distance across the sky ring was purposefully set to $\sim 3$\arcdeg\ to avoid excessive time overheads.
...
The product of this paper is a novel pipeline for reducing CFHT/WIRCam data that is outlined next in \Sec{reduction_outline}, and in subsequent sections.
...
Our confidence in night sky flats as the \textit{correct} choice for flat fielding WIRCam lies in their ability to remove both large scale illumination features (\eg \Fig{qrun_median_sky}) and pixel-to-pixel gain changes across WIRCam (\eg \Fig{flattenedimage_comparison}).
...
\caption{Spatiotemporal evolution of \texttt{FW100K} sky flats, shown as percent difference of the flat fields 15, 30, 60 and 90 minutes after the initial sky flat of the night.
...
With \Fig{qrun_median_sky}, we can easily see that \texttt{QRUN} sky flats produced biased sky shapes---evidently caused by changes in the CFHT/WIRCam optical path and electronics over a queue run, rather than intrinsic variations in the sky itself.

To quantitatively compare the bias of \texttt{QRUN} versus \texttt{FW100K} sky flats, we measure the amplitude of shapes across the $10\arcmin \times 10\arcmin$ WIRCam frame as the 2-standard deviation range of each median sky image's pixel distribution: $2 \sigma(\mathrm{med~sky})$.
These distributions of sky shape amplitude are plotted in \Fig{mediansky_amplitude}.
As expected from \Fig{qrun_median_sky}, \texttt{QRUN} median sky frames have typical biases of 1\% of the sky level, while sky frames from real-time \texttt{FW100K} sky flats are much flatter---$\lesssim 0.3\%$ of the sky level---and more consistently so.

Although \texttt{FW100k} sky flats are better than \texttt{QRUN} flats, we can still ask how accurate they are.
Are the shapes of median sky frames a consequence of intrinsic sky shapes, or of limits in WIRCam flat field accuracy?
We interpret \Fig{mediansky_amplitude} as indicating the latter since 2007B and 2009B median sky images have comparable amplitudes.
Recall that 2009B median sky images are composed of five sky samples taken at random points on the sky, thus potentially sampling uncorrelated sky shapes while the 2007B sky images are composed of sky images taken in rapid succession at the same site on the sky.
Thus we expect the instantaneous shape of the sky (in the sky field) to be captured in the 2007B median sky frames, while the 2009B sky frames should marginalize over five random sky shapes and be flatter by up to a factor of $1/\sqrt{5}$ (for a Gaussian shape distribution).
That this \emph{does not} occur indicates either (a) sky shapes are correlated over tens of minutes and 3\arcdeg\ of sky, or (b) the flatness of median sky images is limited at the 0.3\% level by flat field variations in WIRCam (\eg \Fig{fw100k_movie}).
Indeed, because 2009B median sky images are built over wider time windows than 2007B median sky images (that are built in rapid succession, without target nods interspersed), the intrinsic flat field function of WIRCam can evolve further within 2009B median sky images, offsetting any gains from sampling uncorrelated sky shapes. \todo{Get number for time windows of 2007B and 2009B median sky windows; put in \Sec{mediansky}}.

Also note that in both cases, there are long-tail cases of median sky shape amplitudes as large as 5\% of the sky level \todo{(figure out the circumstances of these!)}.
...
% TODO: what if biases are different?
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/mediansky_amplitude}
\caption{Amplitude of sky level variations across median sky images processed with \texttt{QRUN} and \texttt{FW100K} sky flats.
\comment{Switch to a cumulative distribution to better show scales.}}
...
Yet is difficult to measure the absolute accuracy of real-time sky flats since it is hard to distinguish flat fielding bias from sky shape stochasticity.
Part of this difficulty is that in exclusively observing sky fields the amplitudes of signals due to flat fielding biases or due to sky glow shape stochasticity will be similar.
But if we examine signal- (not sky-) dominated fields, flat fielding biases should grow in comparison to sky shape biases.

In Figures~\ref{fig:frame_residuals_M31-37_Ks_fw100k_medsky} and~\ref{fig:frame_residuals_M31-37_Ks_QRUN} we show the residual shapes of individual WIRCam frames against the median shape of the mosaic, given \texttt{FW100K} and \texttt{QRUN} sky flattening, respectively.
...
\comment{Would it be better to actually plot an RMS map of pixels, rather than slices? Then it would be easier to plot RMS/intensity to plot flat fielding errors}.
...
We also extend previous analysis analysis across the entire dataset.
In \Fig{frame_residuals_M31-37_Ks_QRUN} we plot the distribution of frame-block residual shape amplitudes, measured at the 95\% difference interval.
...
This result is seen for both \texttt{QRUN} and \texttt{FW100K} sky flat pipelines, and for 2007B and 2009B observing schemes.
% TODO
Is this indicative of limitations in flat field accuracy? in median sky accuracy? or in of real sky background fluctuations?
Similarity of amplitudes to unflatness of median sky images indicates a similar process.
In high-SB regions, the shape amplitude is $\sim 0.6\%$ of the sky (\eg \Fig{frame_residuals_m31-37_Ks_fw100k_medsky}), but in lower-SB regions the shape amplitude is $\sim 0.3\%$ of the sky level.

Conclusions:

\begin{itemize}
  \item Frame-by-frame background variability is $\sim 0.3\%$ of sky, seen both in residual differences between frames and blocks on the M31 disk, and in the shapes of sky background images.
  \item No flat fielding technique, nor no observing method, can reliably decrease this variability. Thus it is \emph{impossible} for a WIRCam sky-target nodding program to control the shape of the sky background to better than $0.3\%$ of the sky background.
  \item Field in flat field function increases frame shape variability to $\sim 0.6\%$ of sky.
\end{itemize}
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/frame_residuals/framediffs_skyfrac}
\caption{Distributions of scalar differences between individual frames and blocks in the $J$ (top) and $K_s$ (bottom) mosaics, measured as a dispersion of pixel differences at the 95\% level.
This measurement is a dimensional reduction of the frame residual shape plots (\eg \Fig{frame_residuals_M31-37_Ks_QRUN}), representing the shape variability across $10\arcmin \times 10\arcmin$ WIRCam frames.
}
...
\comment{No correlation seen with the moon angle/illumination. Atmospheric temperature and humidity are also possibilities}.

\comment{Note: \cite{Vaduvescu:2004} and others take this one step further by modelling the sky level as a function of time since sunset as an exponential function: $I = a +b \exp(- k\Delta T)$.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/sunset_airmass}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/pairwise_skyvar_skyfrac}
...
% wircam/skyanalysis/pairwise_skyvariability.py
\end{figure}
...
\comment{Note how bad the 09B sky reliability is!}
