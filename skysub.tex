...
Our examination of frame-block residual differences can also be extended across the entire dataset.
In doing so, we can see how consistently our observing and processing recipes image the disk of M31.
In \Fig{frame_diffs_skyfrac} show the distribution of amplitudes of frame-block residuals, comparing these distributions between filter, sky flat type, and observing run.

\begin{itemize}
  \item negligible difference between 09B and 07B
  \item Similar differences in $J$ and $K_s$ as a fraction of sky level
  \item The disk is most consistently imaged with real-time sky flats. Thus real-time sky flats \emph{are} necessary for tracking the WIRCam flat field function; we cannot band-aid our images with just real-time median sky subtraction.
\end{itemize}
