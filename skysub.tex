...
Further, we also compute $S$, the median of all levels $\alpha_{i,j}$.
Then each the sky image is scaled by the factor $\mathcal{F}_{ij} = \mathrm{median}(\beta_1, \beta_2, \beta_3, \beta_4) / (\alpha_{ij} S)$.
Note that the ratio of $\mathrm{median}(\beta_1, \beta_2, \beta_3, \beta_4) / \alpha_{ij}$ scales each frame to a common level is a detector stack, and unifies the zeropoints across the array.
The factor of $S$ normalizes the sky flat.
In \Fig{fw100k_zpdiff} we show the distribution of flux scale factors in each chip relative to WIRCam detector \#1 (NW quadrant), which can effectively be interpreted as chip-to-chip zeropoint offsets.
